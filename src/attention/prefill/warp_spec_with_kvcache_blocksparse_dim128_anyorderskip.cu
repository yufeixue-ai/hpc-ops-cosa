...
#include "src/attention/prefill/warp_spec_with_kvcache_blocksparse_dim128_anyorderskip.h"
...
void launch_warp_spec_with_kvcache_blocksparse_dim128_anyorderskip(
  ...
  // 1. compute any-order block-sparse attention with threshold skip
  {
    ...
    shm_size += (num_k_block_in_mask + 2) * sizeof(int);
    shm_size += 8 * sizeof(float);
    ...
    kernel<<<grid, block, shm_size, stream>>>(
        ...
        head_kv_divmod, head_q_divmod, tile_m_divmod, (const int32_t *)row_blockmask_ptr,
        num_k_block_in_mask, threshold);
  }
}
...
void warp_spec_with_kvcache_blocksparse_dim128_anyorderskip_async(
    ...
    const void *row_blockmask_ptr, int num_k_block_in_mask, float threshold, cudaStream_t stream) {
  if (block_size == 32) {
    launch_warp_spec_with_kvcache_blocksparse_dim128_anyorderskip<32>(
        ...
        ldV2, row_blockmask_ptr, num_k_block_in_mask, threshold, stream);
  } else if (block_size == 64) {
    launch_warp_spec_with_kvcache_blocksparse_dim128_anyorderskip<64>(
        y_ptr, q_ptr, kcache_ptr, vcache_ptr, cu_seqlens_q_ptr, block_ids_ptr, seqlens_kvcache_ptr,
        tmas_ptr, num_batch, total_seq_q, max_seq_q, num_dim_qk, num_dim_v, num_head_q, num_head_kv,
        num_kvcache_blocks, block_size, num_seq_max_blocks, ldY, ldQ, ldK, ldK1, ldK2, ldV, ldV1,
        ldV2, row_blockmask_ptr, num_k_block_in_mask, threshold, stream);
